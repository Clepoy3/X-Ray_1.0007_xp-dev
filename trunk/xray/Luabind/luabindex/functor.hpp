...
{
	template <class result_type>
	struct default_converter<luabind::functor<result_type>>
		: native_converter_base<luabind::functor<result_type>>
	{
		...
		luabind::functor<result_type> from(lua_State* L, int index)
		{
			if (lua_type(L, index) == LUA_TNIL)
				return luabind::functor<result_type>();
			return luabind::functor<result_type>(object(from_stack(L, index)));
		}

		void to(lua_State* L, luabind::functor<result_type> const& x)
		{
			x.push(L);
		}
	};
	...
	template <class result_type>
	struct default_converter<luabind::functor<result_type> const&>
		: default_converter<luabind::functor<result_type>>
	{};
