...
void	CResourceManager::reset_begin			()
{
	...
	for (auto rtc_it=m_rtargets_c.begin(); rtc_it!=m_rtargets_c.end(); rtc_it++)
...
#pragma todo("container is created in stack!")
		xr_vector<CRTC*>	rt;
		for (auto rt_it=m_rtargets_c.begin(); rt_it!=m_rtargets_c.end(); rt_it++)	rt.push_back(rt_it->second);
