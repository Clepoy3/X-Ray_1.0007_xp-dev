...
void CObjectFactory::register_script	() const
{
	...
	luabind::module				(L)[std::move(instance)]; // это представление нельз€ обработать как таблицу
	...
	//ƒобавлено табличное представление класса сlsid, под именем clsid_table - чтобы была возможность обработки в циклах (object_factory_script.cpp).
