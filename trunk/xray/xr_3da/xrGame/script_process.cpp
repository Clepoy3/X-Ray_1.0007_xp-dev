...
CScriptProcess::CScriptProcess(shared_str name, shared_str scripts)
{
	m_name = name;
//#ifdef DEBUG
	Msg("[CScriptProcess] Initializing %s script process",*m_name); //Посмотреть чего это
//#endif
	string256 I;
...
void CScriptProcess::run_scripts()
{
	...
	for (; !m_scripts_to_run.empty();)
	{
		...
		CScriptThread *script = xr_new<CScriptThread>(S,do_string,reload); //Это переделать?
		xr_free(S);
