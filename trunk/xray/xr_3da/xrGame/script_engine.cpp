...
//-----------------------------------------------------//
	...
	g_game_lua = LSVM; //KRodin: не трогать, пусть так и будет.
...
void log_script_error(const char* format, ...) //Взызвается из трех мест, надо б удалить наверно. Но пока пусть останется.
{
	string1024 line_buf;
	va_list mark;	
