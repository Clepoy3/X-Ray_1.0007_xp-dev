...
#pragma todo("Dima to all : this smart_cast is not optimized!")
	...
	struct CHelper2 {
		...
		IC	static T1* smart_cast(T2* p)
		{
			static_assert(!std::is_const_v<T2> || std::is_const_v<T1>, "Cannot_use_smart_cast_to_convert_const_to_non_const");
...
T1	smart_cast(T2* p)
{
	...
	static_assert(std::is_pointer_v<T1>, "Invalid_target_type_for_Dynamic_Cast");
	static_assert(std::is_void_v<std::remove_pointer_t<T1>> || std::is_polymorphic_v<std::remove_pointer_t<T1>>, "Invalid_target_type_for_Dynamic_Cast");
...
{
#ifdef PURE_DYNAMIC_CAST_COMPATIBILITY_CHECK
	static_assert(std::is_reference_v<T1>, "Invalid_target_type_for_Dynamic_Cast");
	static_assert(std::is_polymorphic_v<std::remove_reference_t<T1>>, "Invalid_target_type_for_Dynamic_Cast");
	static_assert(std::is_polymorphic_v<T2>, "Invalid_source_type_for_Dynamic_Cast");
#endif
#ifdef SMART_CAST_STATS_ALL
