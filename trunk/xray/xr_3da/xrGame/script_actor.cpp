...
void CScriptActor::script_register(lua_State *L)
{
	module(L)
		[
			...
			,
			class_ <CActorConditionObject, bases<CActorCondition, CEntityCondition>>("CActorCondition") // нормальное наследование свойств происходит через Ж ( //KRodin: бля, где враппер???
			,
			...
			,
			class_<CActorObject, bases<CActor, CEntityAlive>>("CActor")	// хак с наследованием нужен для переопределения свойств. Luabind не поддерживает property getters override //KRodin: враппера нет и тут
