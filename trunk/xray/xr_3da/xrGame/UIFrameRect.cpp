...
void CUIFrameRect::UpdateSize()
{
	...
	tile_x 			= iFloor(size_top/ts.x);	tile_x= std::max(tile_x, 0);
	tile_y 			= iFloor(size_left/ts.y);	tile_y= std::max(tile_y, 0);
	...
	tile_x				= iFloor(size_bottom/ts.x); tile_x= std::max(tile_x, 0);
	...
	tile_y				= iFloor(size_right/ts.y); tile_y= std::max(tile_y, 0);
